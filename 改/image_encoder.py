...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        self.mamba_layers = VSSLayer(dim=768,drop_path=0.35, attn_drop_rate=0.1,depth=7)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        mambax = x
        mambax =  self.mamba_layers(mambax)

        for blk in self.blocks:
            x , mambax = blk(x, mambax)
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mambax: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.window_size == 0:
            sax = self.Space_Adapter(mambax, x, x)  # b h w c
            x = x + sax
