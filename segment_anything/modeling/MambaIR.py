...
class VSSBlock(nn.Module):
    ...
    def forward(self, input):
        B, H, W, C = input.shape  # 直接从输入获取所有维度信息
        x = self.ln_1(input)
        x = input * self.skip_scale + self.drop_path(self.self_attention(x))
        x = x * self.skip_scale2 + self.conv_blk(self.ln_2(x).permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3,1).contiguous()
        return x  # 直接返回处理后的数据，保持[B, H, W, C]的形状


# if __name__ == '__main__':
#     # 初始化VSSBlock模块，hidden_dim为128
#     block = VSSBlock(hidden_dim=768, drop_path=0.1, attn_drop_rate=0.1, d_state=16, expand=2.0, is_light_sr=False)
#
#     # 将模块转移到合适的设备上
...
#
#     # 计算输出
#     output_tensor = block(input_tensor)  # 由于输入已经是 [B, H, W, C]，直接传递即可
#
#     # 打印输入和输出张量的尺寸
