...
def parse_args():
    ...
    parser.add_argument("--metrics", nargs='+', default=['iou', 'dice','f_beta','mae'], help="metrics")
    parser.add_argument("--model_type", type=str, default="vit_b", help="sam model_type")
    parser.add_argument("--sam_checkpoint_dir", type=str, default="workdir/models/sam-med2d",
                        help="sam checkpoint directory")
...
def test_model(args, checkpoint):
    print(f'Testing model checkpoint: {checkpoint}')
    args.sam_checkpoint = checkpoint
...
def main(args):
    ...
    for epoch in range(1, 17):
        checkpoint = os.path.join(args.sam_checkpoint_dir, f'epoch{epoch}_sam.pth')
        if os.path.exists(checkpoint):
            loss, metrics = test_model(args, checkpoint)
            if best_metrics is None or metrics['dice'] > best_metrics['dice']:
                best_metrics = metrics
                best_checkpoint = checkpoint

    print(f'Best checkpoint: {best_checkpoint}, Best metrics: {best_metrics}')
